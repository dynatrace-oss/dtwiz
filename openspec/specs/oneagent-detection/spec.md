# oneagent-detection Specification

## Purpose

Define how `pkg/analyzer` decides whether OneAgent is present and running on the
host, so that the reported state is correct on both systemd hosts and on
containers where `systemctl` may be a compatibility shim.
