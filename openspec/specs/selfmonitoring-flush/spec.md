...
Guarantee that self-monitoring events reach the tenant before the process exits, on every exit
path, so that fast-failing commands, cancellations, and interruptions are not systematically lost
from the funnel. The wait is bounded and invisible to the user.
