...
A formal `InstrumentationPlan` interface is defined in `otel.go` and implemented by all runtime plan structs:

```go
type InstrumentationPlan interface {
    Runtime() string           // display name, e.g. "Python", "Java"
    PrintPlanSteps()           // render preview lines for the combined plan
    Execute()                  // run the actual instrumentation after collector install
}
```

The orchestrator stores the selected plan as `InstrumentationPlan` and calls its methods directly. Only one plan is active per invocation. The `Runtime()` method returns the display name used in headers (e.g. `── Python auto-instrumentation ──`).

**Alternative considered:** No formal interface — orchestrator calls plan methods via nil-check only. Rejected in favour of the interface because it enables the orchestrator to store any runtime's plan in a single typed variable and call `Runtime()` generically for separator headers.
...
When `--dry-run` is set, `InstallOtelCollector` exits immediately after printing the collector dry-run plan — before project scanning begins. No project list or runtime plan is shown. The dry-run path is handled by `prepareCollectorPlan` + `cp.printDryRun()` and returns before reaching the project detection logic.
