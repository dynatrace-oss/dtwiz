...
- [x] 3.16 Wire `opts.ConnectivityCheckOnly` (from `InstallOptions`): if `true`, call `CheckAllEndpoints`, print a table-format report (`host:port  reachable  latency  error`), then return `nil` without proceeding to token minting, download, or install
