...
- **Foreign-collector fallback.** When a non-dtwiz collector is already running such that reliable in-place merge is not feasible, deploy a **separate** dtwiz-managed host-monitoring collector rather than restructuring the third-party config (free-port probing already supports multiple collectors on one host).
...
- `otel-host-monitoring`: `install otel` deploys host metrics and host logs collection through the managed OTel Collector, using the Dynatrace Host Monitoring reference configuration, merged into a single managed collector where possible and a separate collector otherwise, with platform-aware signal selection.
...
- **Detection:** reuse `findDynatraceOtelCollectors` and `findAllRunningOtelCollectors` to decide combined-config versus separate-collector.
