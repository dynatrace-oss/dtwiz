...
- New `logs/host` pipeline (Linux only): journald to resource_detection to otlp_http. The existing `logs` pipeline (otlp-only) is retained on all platforms for application logs; `logs/host` is a separate host-log pipeline and does not receive from `otlp`, so OTLP logs are never duplicated.
...
The `logs/host` pipeline is Linux-only: it exists solely to collect journald logs, and `journald` is unavailable on macOS and Windows. Per Dynatrace docs, a `journald` receiver present on a non-Linux OS causes the Collector to exit on startup, so both the receiver definition and the pipeline must be absent on those platforms. Application OTLP logs continue to flow through the existing `logs` pipeline on all platforms, which keeps `logs/host` a pure host-log concern with no duplication. A small config-data struct field (`IncludeJournald bool`) gates the `journald` receiver definition and its `logs/host` pipeline together so the two can never drift out of sync.
...
- **Cross-platform log coverage:** host log collection (`logs/host` via `journald`) is Linux-only; macOS and Windows have no `logs/host` pipeline. Native platform log receivers (`windowseventlogreceiver`, `macosunifiedloggingreceiver`) are not in the Dynatrace distribution and cannot be added by dtwiz (see Decision 3 and task 0.3). Application OTLP logs continue to reach Dynatrace on all platforms through the `logs` pipeline.
