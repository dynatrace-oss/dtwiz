...
Update reconciles **only** the `da-azure` monitoring configuration to the latest schema-derived defaults and leaves the entire authentication chain unchanged: connection, Service Principal, federated credential, and Monitoring Reader role assignment. It discovers the existing configuration and the bound connection, resolves the active subscription, and rewrites the configuration with a single update call (or creates one when none exists), sharing the same body-builder logic with install.
