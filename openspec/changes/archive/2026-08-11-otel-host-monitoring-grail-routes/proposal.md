...
- **Safe by default.** dtwiz resolves the target "OpenTelemetry Host Monitoring" pipeline by name at runtime, right before applying the routes rather than from the earlier preview. This means a signal shown as pending in the preview can still get its route created once the extension's pipeline appears. dtwiz also waits briefly for the pipeline to become listable before checking. If it is still not found, dtwiz skips that route with a message pointing at a re-run, instead of telling the user to activate the extension themselves, and does not fail the install. See `design.md` Decision 8 for the full behavior.
...
- **Extension status previewed too.** Since the routes are meaningless without the extension's pipeline, and extension activation runs before the routes are applied, the install preview also shows a read-only line for the extension itself (already installed / will be installed and activated), ordered before the route plan section. It doesn't claim a specific activation outcome for an installed extension, since the Dynatrace API's activation flag isn't a reliable signal for this extension. This makes no install or activation call by itself; see `design.md` Decision 7.
