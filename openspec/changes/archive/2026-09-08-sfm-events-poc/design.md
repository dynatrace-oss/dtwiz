...
- Use a package-level `http.Client` with a 3-second timeout rather than `http.DefaultClient` or the typed `client.Client`.
  - Rationale: the selfmonitoring package is a PoC with no dependency on the installer client stack. Using the typed client would couple the package to credential resolution, which is handled one level up in `cmd/root.go`. A dedicated client with a short timeout bounds the async send operation.
...
- Credential resolution and HTTP send run in a background goroutine to avoid blocking `PersistentPreRun`. Event parameters are captured synchronously before the goroutine is launched. The 3-second timeout bounds the async send operation.
