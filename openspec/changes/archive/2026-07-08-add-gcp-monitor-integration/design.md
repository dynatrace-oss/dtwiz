...
Connections found by `findAllConnections` are split into *complete* (carry a bound service-account email) and *incomplete* (created in step 2 but never finalized in step 6, from an interrupted prior run):
