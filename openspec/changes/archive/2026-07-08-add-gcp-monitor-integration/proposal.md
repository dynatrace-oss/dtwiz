...
- **Update via `dtwiz setup` and `dtwiz update gcp`**: when a complete GCP connection already exists, the setup/update flow reconciles the monitoring configuration in place using the latest defaults and leaves the authentication chain untouched.
