...
- [x] 2.1 Add `JavaProject` struct and `detectJavaProjects()` using `scanProjectDirs` for `pom.xml`, `build.gradle`, `build.gradle.kts`
- [x] 2.2 Add `detectJavaProcesses()` and `matchJavaProcessesToProjects()` using shared `detectProcesses`/`processMatchPIDs`
- [x] 2.3 Define `JavaInstrumentationPlan` struct with project, env vars, `EnvURL`, `PlatformToken`
...
- [x] 3.4 Define `NodeInstrumentationPlan` struct and `generateOtelNodeEnvVars()`
...
- [x] 4.2 Define `GoInstrumentationPlan` struct and `generateOtelGoEnvVars()`
