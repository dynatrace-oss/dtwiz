...
- [x] 1.4 Update `pkg/installer/otel/update_dynatrace.go` so regenerated Dynatrace Collector configs always include host-monitoring settings.
- [x] 1.5 Update `pkg/installer/otel/collector.go` `InstallOtelCollectorOnly` to run the extension activation preview, OpenPipeline route preview, extension activation, pipeline wait, and route application in the same pattern as `InstallOtel`.
...
- [x] 2.2 Rewrite `pkg/installer/otel/otel_test.go` install/uninstall tests that assert experimental-gated activation, messaging, route setup, and deactivation so they assert regular behavior and dry-run safety.
- [x] 2.3 Update OpenSpec deltas for `otel-host-monitoring`, `otel-host-group-id`, `otel-extension-activation`, `otel-host-monitoring-grail-routes`, `otel-extension-deactivation`, and `otel-collector-update` to remove experimental flag requirements and any host-monitoring config toggle.
