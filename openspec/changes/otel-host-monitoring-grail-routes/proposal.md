...
- **Extension activation status previewed too.** Since the routes are meaningless without the extension's pipeline, and extension activation runs before the routes are applied, the install preview also shows a read-only line for the extension itself (already active / will be activated / will be installed and activated), ordered before the route plan section. This makes no install or activation call by itself; see `design.md` Decision 7.
