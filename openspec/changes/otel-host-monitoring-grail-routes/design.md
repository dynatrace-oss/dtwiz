...
This was arrived at after three rejected approaches, all worth recording so they aren't retried:
...
- **Extension-ownership via `externalId` prefix:** the implementation relies on the extension's pipeline objects carrying an `externalId` starting with `com.dynatrace.extension.opentelemetry_` (Decision 2). This is a platform-assigned field derived from the extension's registered name, giving an unambiguous reference to the extension's identity that does not depend on what the extension writes inside `value`. If a future extension rename or re-registration changes the `externalId` prefix, `checkPipeline` finds no owned object and the route is skipped safely. Earlier approaches that were tried and rejected: filtering by `externalIds` query parameter (requires hardcoding per-signal suffixes); matching by `value.metadataList` `extension_name` (extension-internal metadata, less reliable than the platform-assigned `externalId`); using `value.customId` directly as the routing `pipelineId` (`400 Must be of type setting`).
