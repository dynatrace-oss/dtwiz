...
- **Whole-object PUT clobbers sibling routes:** because a signal's routes live in one object, a PUT that omits existing `routingEntries` would disable unrelated user routes. The live spans object already contains a user an unrelated user route next to ours. Mitigation: append-only construction that carries every prior entry, plus a unit test asserting sibling entries survive (task 6.1); optimistic `If-Match` guards against concurrent edits.
