...
- [x] 2.1 Create `pkg/installer/otel_process_windows.go` (`//go:build windows`) with `pythonChildPIDs(parentPID int) ([]int, error)` using `Get-CimInstance Win32_Process` via PowerShell, filtering by `ParentProcessId` and exe name containing `python`
- [x] 2.2 Add `adoptExeclChildren(procs []*ManagedProcess, started *int, notStarted *int)` to `otel_process_windows.go` — iterates processes with clean exits, calls `pythonChildPIDs`, picks lowest PID if multiple children, updates `ManagedProcess.PID`, resets exit state fields, wires `watchPID`, adjusts counters, emits `logger.Debug` for all outcomes (no child, multiple children, success, `OpenProcess` failure)
...
- [x] 3.6 Extract `winProcessQuery(whereClause, fieldsExpr string) []string` helper in `pkg/installer/otel_runtime_scan_windows.go` — owns PowerShell invocation, CRLF trimming, and blank-line filtering; update `detectProcesses`, `pythonChildPIDs` (`otel_process_windows.go`), and `findRunningOtelCollectors` (`otel_collector_windows.go`) to use it; per-PID scalar lookups and `pkg/analyzer` callers are left as direct `exec.Command` calls (different shape / different package)
...
- [x] 4.4 Add Windows-only adoption logic tests in `pkg/installer/otel_process_windows_test.go` (`//go:build windows`): `adoptExeclChildren` with all-running processes (no change), with crashed process (skipped), with clean-exited process and no Python children (no adoption)
