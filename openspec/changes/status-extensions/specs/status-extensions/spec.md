...
The system SHALL provide a `client.New()` constructor in `pkg/client/` that returns a `Client` containing a `ClassicClient` (Classic API, token auth) and a `PlatformClient` (Platform/Apps API, `Bearer` auth). Both sub-clients SHALL share the same retry policy, timeout, and debug-logging behaviour. The access token is optional; when absent, the platform token is used for Classic API calls as well.
...
- **WHEN** `client.New()` is called with resolved URLs and tokens
...
- **WHEN** credential resolution runs before calling `client.New()`
